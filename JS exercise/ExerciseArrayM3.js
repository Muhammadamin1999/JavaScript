...
// const UserIDofUser = (users)=> users.filter((el)=> el.firstName === 'Madison' && el.lastName ==='Marshall')
// const [user] = UserIDofUser(users);
// console.log(user.id);

// 2) Who wrote the first comment (assuming the first comment is in position 0 of the comments array)
// const who = (users, comments) => users.find(user=> user.id === comments[0].userId);
// console.log(who(users, comments))


// 3) Which user commented 'OK great thanks'?

// const AuthorOfComment = (users,comments) => users.find(user=> user.id === comments.find(comment=> comment.text === "OK great thanks").userId)
// console.log(AuthorOfComment(users,comments))

// 4) Add the user's first and last name to each comment in the comments array
// const AddUser = (users, comments) => comments.map(comment=>
//     {
...
//     )
// console.log(AddUser(users, comments))


// 5) Get a list of the users who haven't commented

const NotCommented = (users, comments)=> users.filter(user=>
    !comments.find(comment=> comment.userId === user.id))

console.log(NotCommented(users, comments))