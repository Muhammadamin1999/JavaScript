...
// const isValid = (users)=> users.every(user=> /[^@]+@[^\.]+\..+/.test(user.email));


// console.log(isValid(users));
...
//   const IpAdress = (users)=> users.filter(({ip_address})=>ip_address.split('.')[0] < 128);
//   console.log(IpAdress(users));
  //({ip_address}) is the destructuring pattern where you specify which property you want to extract from each object in the array.
...
// const firstIndexofIP = (users)=> users.filter(({ip_address})=> ip_address.split('.')[0]<128)[0]
// console.log(firstIndexofIP(users));
...
// const replaceItem = (users)=> users.fill('deleted');
// console.log(replaceItem(users));
